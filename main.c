/*
  ...
  * Programa: SISTEMA DE REGISTO E CONTROLE DE FUNCIONÁRIO
  * Data de criação: 27/04/2020
  * Participante: Andreia Gonçalves Semedo
  * Finalizado em: 15/05/2020
...
void mostrarSalario(){
    ...
    if ( CALCULARSALARIO == true ) {
        ...
        printf("O salario do funcionario é: %.1f $\n",  (float)calcularSalario ( 8, valorHora ));
        getchar();
...
void gerarArquivoProva() {
    ...
    if ( file == NULL ) {
        ...
        Dados d[10] = {
            { "Pedro", "123456002", "Docente", 0,37},
            { "Ana" , "123456010", "Tecnico", 0, 5 },
            { "Suzi", "123456007", "Tecnico", 0, 10},
            { "Fred", "123456001", "Diretor", 0, 8 },
            { "Rute", "123456003", "Docente", 0, 20},
            { "Rosi", "123456006", "Docente", 0, 16},
            { "Jose", "123456004", "Docente", 0, 3 },
            { "Raul", "123456005", "Diretor", 0, 60},
            { "Beto", "123456009", "Docente", 0, 10},
            { "Jaco", "123456008", "Tecnico", 0, 4 }
        };
        ...
        for ( i = 0; i < 10; i++ ){ /*Para cada funcionario da lista de testes...*/
            ...
            if(CALCULARSALARIO == true){
                ...
                strcpy( str, d[i].categoria );    /*Copiamos a categoria do funcionario para a string str, para processa-la(converter em minusculas)*/
...
/*Função que calcula o salario em base ao numero de horas multiplicado pelo valor que cada categoria cobra. Logo se aplica imporstos*/
int calcularSalario( int nHoras, int valorHora ){
    int salarioLiquido = (nHoras * valorHora) ;                    /*Salario sem aplicar impostos*/
...
int cargarFuncionarios( Dados **lista, int * ptr_nFuncionarios, bool liberarLista ){
    ...
    *ptr_nFuncionarios = 0;
    ...
    /* Reserva memoria para a lista e lê o arquivo.
        Reserva conforme o numero de usuarios contados no arquivo. Se não existe o arquivo, ou não
    ...
    if( *ptr_nFuncionarios > 0 ){
        *lista = malloc( sizeof(Dados) * *ptr_nFuncionarios );
        ...
        if( *lista != NULL && file != NULL ){
            rewind(file);
            /*Caregamos os usuarios do arquivo na lista*/
            for ( i=0; i < *ptr_nFuncionarios; i++) {
...
void imprimirRegistros ( Dados **lista, int * ptr_nFuncionarios, int minimo ) {
    /*carega/mos o/s funcionario/s na lista se tiver 1 ou mais funcionario/s no arquivo*/
...
/*Função que carega os funcionarios do arquivo para a lista dinâmica e passa a lista para a função "mostrar",
  para que imprima os funcionarios por categoria
*/
void imprimirCategoria(Dados **lista, int * ptr_nFuncionarios){
    /*carega/mos o/s funcionario/s na lista se tiver pelo menos um, ou mais funcionario/s no arquivo*/
    int n = cargarFuncionarios ( &*lista, &*ptr_nFuncionarios, false );
    if (  n > 0 ) {
        limparTela();
        system ( "COLOR 0F" );
...
void eliminar(){
    ...
    limparTela();
    system ( "COLOR 0F" );
    printf("Informe o NIF que deseja pesquisar: ");
    scanf("%s", nif);
        ...
        if ( d.nome[0] != ' ' ) {
            ...
            if ( strcmp ( d.NIF, nif ) == 0 ) {
                ...
                switch ( opt ) {
                    ...
                    case 0:/*Volta para o menu(cai no default porque não tem break)*/
                    default:
...
/*Função que ordena o funcionarios.txt por salario de maior a menor e envia esses dados a outro arquivo chamado ordenar_funcionarios.txt*/
void ordenarRegistros(Dados **lista, int * ptr_nFuncionarios){
    *ptr_nFuncionarios = 0;
    int i , z;
    ...
    FILE * file = fopen ( "funcionarios.txt", "r" );
    if ( file == NULL ) {/*Se o arquivo não abriu*/
        perror ( "Erro ao tentar abrir funcionarios.txt" );
    ...
    /*Caregamos os funcionarios na lista*/
